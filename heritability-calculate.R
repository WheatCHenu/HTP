...
Onewaybluph2NEW<-function(x){
  ##Ca  ##Calculate variance components
    ...
    cat('----OUTPUT RESULTS OF EACH TRAITS INTO FILES:----','\n','----BroadSenseHeritability.txt----','\n',
        '----BlupEachlines.txt----','\n')
    write.table(heritability,'test2.txt',sep='\t',quote=F)
    write.csv(line.blup,'heritability167-111g.csv22',row.names=F)
}

x<-read.table("test.txt",header=T,sep="\t")
Onewaybluph2NEW(x)
